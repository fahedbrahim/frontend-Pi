...
class ListeLivraison extends Component {
  ...
  onDelete = (row) => {
    axios
      .put(`/provider/refuseprovider/${row.original.id}`)
      .then((res) => {
        ...
        <ReactTable
          ...
          columns={[
            ...
            },
            {
              ...
              accessor: "State",
              Cell: (row) => {
                console.log("éé", row.original);
                return (
                  <div className="d-block w-100 text-center">
                    {row.original.State === "Affected" ? (
                      <>
                        {" "}
                        <div
                          data-tip="Your delivery is accepted and affected"
                          data-for="Utilisé"
                          data-place="top"
                          className=" w-75 badge badge-success "
                        >
                          {row.original.State}
                        </div>
                        <ReactTooltip backgroundColor="#41c47d" id="Utilisé" />
                      </>
                    ) : (
                      <>
                        {" "}
                        <div
                          data-tip="Waiting for  provider"
                          data-for="else"
                          data-place="top"
                          style={{ color: "white" }}
                        ...
                        <ReactTooltip backgroundColor="#f7b924" id="else" />
                      </>
                    )}
                  </div>
                );
