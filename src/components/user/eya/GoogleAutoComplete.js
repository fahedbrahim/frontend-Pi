...
export default class GoogleAutoComplete extends React.Component {
  ...
  handleSearchClear(searchText) {
    if (searchText.target.type === "search") {
      if (searchText.target.value === "") {
        ...
        this.props.callbackClearFunction(null);
      }
    }
  }
  ...
  handleMapChange() {
    ...
    if (this.place.address_components) {
      ...
      var matchForStreet1 = this.place.adr_address
        ? this.place.adr_address.match(
            /<span class="street-address">(.*?)<\/span>/
          )
        : false;
      if (matchForStreet1 && matchForStreet1[1])
        fieldsForState.streetAddress = matchForStreet1[1];
      ...
      Object.keys(addrComps).map((index) => {
        ...
        switch (addrType) {
          ...
          case fields.cityOrState:
            fieldsForState.cityOrState = addrComps[index].long_name;
            break;

          case fields.country:
            fieldsForState.country = addrComps[index].long_name;
            break;
  ...
  render() {
    ...
    return [
      ...
      <div className={`address ${this.state.showResult ? "showFields" : ""}`}>
        ...
        <div className="addressFields">{this.renderFields()}</div>
      </div>,
    ];
  }
}
